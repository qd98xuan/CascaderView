 ...
 * type 0--确定1--新加2--人员3--一人一档
 */
class NodeTreeAdapter(context: Context, listView: ListView, linkedList: LinkedList<Node<*>>,isHideAddButton:Boolean,onTreeClickCallback: OnTreeClickCallback) :
    BaseAdapter() {
    ...
    private lateinit var onTreeClickCallback:OnTreeClickCallback//点击回调

    private var isHideAddButton = false
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View? {
        ...
        if (convertView == null) {
            ...
            holder = ViewHolder()
            holder.imageView = convertView.findViewById(R.id.id_treenode_icon) as ImageView
            holder.label = convertView.findViewById(R.id.id_treenode_label) as TextView
            holder.confirm = convertView.findViewById(R.id.id_confirm) as LinearLayout
            holder.add = convertView.findViewById(R.id.id_add) as LinearLayout
            holder.peoplegear = convertView.findViewById(R.id.id_people_gear) as LinearLayout
            holder.person = convertView.findViewById(R.id.id_person) as LinearLayout
        ...
        holder.confirm!!.setOnClickListener { v ->
            onTreeClickCallback.onItemClickListener(node.get_id().toString(),node.get_parentId().toString(),node.get_label().toString(),0)
        }
        if (isHideAddButton){
            holder.add!!.visibility = View.GONE
            holder.person!!.visibility = View.GONE
            holder.peoplegear!!.visibility = View.GONE

        }else{
            holder.add!!.visibility = View.VISIBLE
            holder.person!!.visibility = View.VISIBLE
            holder.peoplegear!!.visibility = View.VISIBLE
        }
        holder.add!!.setOnClickListener { v->
            onTreeClickCallback.onItemClickListener(node.get_id().toString(),node.get_parentId().toString(),node.get_label().toString(),1)
        }
        holder.person!!.setOnClickListener { v->
            onTreeClickCallback.onItemClickListener(node.get_id().toString(),node.get_parentId().toString(),node.get_label().toString(),2)
        }
        holder.peoplegear!!.setOnClickListener { v->
            onTreeClickCallback.onItemClickListener(node.get_id().toString(),node.get_parentId().toString(),node.get_label().toString(),3)
        }
    ...
    internal class ViewHolder {
        ...
        var peoplegear: LinearLayout? = null
    }
    ...
    init {
        ...
        this.isHideAddButton = isHideAddButton
...
interface OnTreeClickCallback{
    fun onItemClickListener(id:String,parentId:String,name:String,type:Int)
}